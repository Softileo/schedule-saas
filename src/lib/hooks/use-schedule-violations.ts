...
/**
 * Konfiguracja włączania/wyłączania poszczególnych sprawdzeń naruszeń
 */
export interface ViolationChecksConfig {
    /** Sprawdzanie minimalnej obsady (domyślnie: true) */
    staffingMin?: boolean;
    /** Sprawdzanie maksymalnej obsady (domyślnie: true) */
    staffingMax?: boolean;
    /** Sprawdzanie kolizji z nieobecnościami (domyślnie: true) */
    absence?: boolean;
    /** Sprawdzanie 11h odpoczynku dobowego (domyślnie: true) */
    rest11h?: boolean;
    /** Sprawdzanie 35h odpoczynku tygodniowego (domyślnie: true) */
    rest35h?: boolean;
    /** Sprawdzanie przekroczenia 8h/dobę (domyślnie: true) */
    dailyHours?: boolean;
    /** Sprawdzanie przekroczenia 48h/tydzień (domyślnie: true) */
    weeklyHours?: boolean;
    /** Sprawdzanie max dni pracy z rzędu (domyślnie: true) */
    consecutiveDays?: boolean;
}

/** Domyślna konfiguracja - wszystkie sprawdzenia włączone */
export const DEFAULT_VIOLATION_CHECKS: Required<ViolationChecksConfig> = {
    staffingMin: true,
    staffingMax: true,
    absence: true,
    rest11h: true,
...
interface UseScheduleViolationsProps {
    ...
    /** Konfiguracja włączania/wyłączania sprawdzeń (domyślnie wszystkie włączone) */
    checksConfig?: ViolationChecksConfig;
}
...
export function useScheduleViolations({
    ...
    const violations = useMemo(() => {
        ...
        const config: Required<ViolationChecksConfig> = {
            ...DEFAULT_VIOLATION_CHECKS,
            ...checksConfig,
        };
        ...
        if (
            ...
            (config.staffingMin || config.staffingMax)
        ) {
            ...
            shiftsByTime.forEach((shifts, key) => {
                ...
                if (template) {
                    if (config.staffingMin && count < template.min_employees) {
                        result.push({
                            type: "staffing_min",
                            employeeId: "system",
        ...
        employees.forEach((emp) => {
            ...
            if (config.absence) {
                ...
                employeeShifts.forEach((shift) => {
                    ...
                    const conflictingAbsence = employeeAbsences.find(
                        (absence) => {
                ...
                employeeShifts.forEach((shift) => {
                    ...
                    if (hours > MAX_DAILY_WORK_HOURS) {
                        ...
                        result.push({
                            ...
                            employeeName,
                            description:
                                LABOR_CODE_VIOLATION_MESSAGES.DAILY_HOURS,
            ...
            if (config.weeklyHours || config.rest35h) {
                ...
                shiftsByWeek.forEach((weekShifts, weekStart) => {
                    ...
                    if (
                        config.weeklyHours &&
                        totalHours > MAX_WEEKLY_WORK_HOURS_WITH_OVERTIME
                    ) {
                        result.push({
                            type: "weekly_hours",
                            employeeId: emp.id,
                    ...
                    if (config.rest35h) {
                        ...
                        const sortedWeekShifts = [...weekShifts].sort(
                            (a, b) => {
                    ...
                    } // zamknięcie config.rest35h
                });
            } // zamknięcie config.weeklyHours || config.rest35h
            ...
            } // zamknięcie config.consecutiveDays
        });
        ...
        return uniqueResult;
    }, [employees, activeShifts, absences, shiftTemplates, checksConfig]);
